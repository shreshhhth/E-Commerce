  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (!image1 && !image2 && !image3 && !image4) {
        toast.error("Please upload atlease one image")
        return;
      }
      ...
      {/*If only 1 image is there then we dont need to append the other images */}
      ...
      if (response.data.success) {
        toast.success(response.data.message);{/*will display the message that product is added or not added */}
        {/*After this we will reset all the feilds and their states for further use */}
        setName('');
        setDescription('');
        setPrice('');
        setImage1(false);
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
        ...
        <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
          ...
          <div>
            ...
            <input onChange={(e)=>setPrice(e.target.value)} value={price} className='w-full px-3 py-2 sm:w-[120px] appearance-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none' type="Number" placeholder='25' />
          </div>
        </div>
        <div>
        ...
        <button className='w-28 py-3 mt-4 bg-black text-white rounded-full' type="submit">Add</button>
    </form>
  )
}
